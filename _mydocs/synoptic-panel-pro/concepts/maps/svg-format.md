...
- Link an element of your map to a specific data point. This can be done by setting the `id` attribute of an SVG element to the target data point value. See more on [Data Binding](../data-binding.md).
...
|---                        |---                    |---|
|`id`                       |Any shape or group     |*(String)* The unique identifier of the element. This attribute can be used to [automatically bind the element to a data point](../data-binding.md).  |
...
Groups are used to group multiple shapes together. They can be used to define **areas** in the map. The `<g>` tag is used to create groups.
